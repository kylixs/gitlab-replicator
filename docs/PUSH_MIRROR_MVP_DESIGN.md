...
erDiagram
    ...
    SOURCE_PROJECT_INFO {
        ...
        string path_with_namespace "项目完整路径"
        string group_path "分组路径"
        string name "项目名称"
        ...
        json metadata "其他元数据(JSON)"
        timestamp synced_at "信息同步时间"
        timestamp updated_at
    }
    ...
    TARGET_PROJECT_INFO {
        ...
        string status "状态: not_exist/creating/created/ready/error/deleted"
        timestamp last_checked_at "最后检查时间"
        string error_message "错误信息"
    ...
    SYNC_EVENT {
        int id PK
        int sync_project_id FK
        string event_type "事件类型: push_detected/sync_started/sync_finished/sync_failed/mirror_created/mirror_updated"
        string event_source "事件来源: webhook/polling/manual/system"
        string status "状态: success/failed/running"
        string commit_sha "提交SHA"
        string ref "引用(分支/标签)"
        string branch_name "分支名称"
        int duration_seconds "耗时(秒)"
        string error_message "错误信息"
        json event_data "事件详细数据"
...
```mermaid
graph TD
    A[GitLab Push Event] --> B[Webhook 接收]
    B --> C[验证签名]
    C --> D[解析事件载荷]
    D --> E[查找对应的 Mirror]
    E --> F{找到 Mirror?}
    F -->|是| G[记录 push_detected 事件]
    F -->|否| H[忽略]
    G --> I[等待轮询确认同步状态]
...
graph TD
    A[查询同步事件] --> B{事件类型}
    B -->|push_detected| C[显示推送信息]
    B -->|sync_finished| D[显示同步成功]
    B -->|sync_failed| E[显示错误信息]
    C --> F[计算推送到同步的延迟]
    D --> G[计算同步耗时]
    E --> H[提供修复建议]
